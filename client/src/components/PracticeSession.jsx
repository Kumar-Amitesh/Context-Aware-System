...
const sessionLabel = (type) => {
  ...
  if (type === 'video_full') return '🎥 Video Interview';
...
const PracticeSession = ({
  ...
  const voiceMode = sessionMode === 'voice';
  ...
  const primaryLabel = videoMode ? 'Start Full Video Interview'
    : voiceMode ? 'Start Full Voice Interview'
    : 'Generate Full Practice Test';

  const primaryIcon = videoMode ? <Send size={18} />
    : voiceMode ? <Mic size={18} />
    : <Send size={18} />;

  const weakLabel = videoMode ? 'Video Interview · Weak Topics'
    : voiceMode ? 'Voice Interview · Weak Topics'
      ...
      <div>
        <div className="card">
            ...
            {(voiceMode || videoMode) && (
              <div style={{ marginTop: 6 }}>
                <span className="badge badge-purple" style={{ fontSize: 12 }}>
                  ...
                  {videoMode ? 'Video Interview Mode' : 'Voice Interview Mode'}
                </span>
              </div>
            )}
            <p style={{ marginTop: 6 }}>
          ...
          <div className="action-stack">
            ...
            >
              <Settings size={14} /> New Session with Different Config
            </button>
          </div>
        </div>
      </div>
...
// import React, { useState } from 'react';
// import { Send, Upload, AlertCircle, FileText, History, Clock, BarChart3, Zap, Mic, Settings, Layers, ChevronDown, ChevronRight } from 'lucide-react';
// import { chatAPI } from '../services/api';
...
//   onGenerateVideoFull, onGenerateVideoWeak,
...
//   const videoMode = isVideoMode(chat);
...
//   const handleVideoFull = async () => {
//     setGenerating(true);
//     try { await onGenerateVideoFull(); } finally { setGenerating(false); }
//   };

//   const handleVideoWeak = async () => {
//     if (!chat?.weakTopics?.length) { alert('No weak topics identified yet. Complete a session first.'); return; }
//     setGenerating(true);
//     try { await onGenerateVideoWeak(); } finally { setGenerating(false); }
//   };
...
//                 >
//                   <Send size={18} />
//                   {generating ? 'Generating…' : 'Start Full Video Interview'}
//                 </button>
//                 <button
//                   className="btn btn-outline btn-full"
//                   onClick={handleVideoWeak}
//                   disabled={generating || !canGenerate || !chat?.weakTopics?.length}
//                 >
//                   <Zap size={16} />
//                   Video Interview · Weak Topics
//                 </button>
//               </>
//             ) : voiceMode ? (
//               <>
...
// import React, { useState } from 'react';
// import { Send, Upload, AlertCircle, FileText, History, Clock, BarChart3, Zap, Mic, Settings, Layers, Video } from 'lucide-react';
// import { chatAPI } from '../services/api';
...
//                   <Video size={11} style={{ marginRight: 4 }} /> Video Interview Mode
//                 </span>
//               </div>
//             )}
//             <p style={{ marginTop: 6 }}>
...
//                 disabled={!hasSessionHistory}
//                 title={!hasSessionHistory ? 'Complete a session first' : 'View analytics'}
//                 style={{ flexDirection: 'column', gap: 4, padding: '10px 6px', height: 'auto' }}
//               >
//                 <BarChart3 size={16} style={{ color: 'var(--primary)' }} />
//                 <span style={{ fontSize: 11 }}>Analytics</span>
//               </button>
...
//                 <span style={{ fontSize: 11 }}>Flashcards</span>
//               </button>

//               {/* Video Practice */}
//               <button
//                 className="btn btn-ghost btn-sm"
//                 onClick={() => onOpenVideoQuestion?.()}
//                 title="Record a video answer"
//                 style={{ flexDirection: 'column', gap: 4, padding: '10px 6px', height: 'auto' }}
//               >
